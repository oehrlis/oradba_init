...
# Check for a valid SID
    ...
    echo "INFO: Add ${newSID} to oratab $ORATAB"
    echo "${newSID}:${ORACLE_HOME}:Y" >>$ORATAB
else
    echo "INFO: ${newSID} already exists in oratab $ORATAB"
fi
...
if [ -f "$HOME/.BE_HOME" ]; then
    ...
    . ${BE_HOME}/bin/oraenv.ksh ${newSID}           # source SID environment
    set -o errexit 
else   
